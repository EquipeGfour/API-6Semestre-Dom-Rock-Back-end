...
@router.get("/users/{user_id}", description="Rota para buscar um usuário pelo ID")
def get_user(user_id: int, db: Session = Depends(get_db)):
    return UsersController().get_user(user_id, db)

@router.get("/users", description="Rota para obter todos os usuários")
...
@router.put("/users/{user_id}", description="Rota para atualizar os dados de um usuário")
...
@router.delete("/users/{user_id}", description="Rota para excluir um usuário pelo ID")
